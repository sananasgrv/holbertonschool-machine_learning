#!/usr/bin/env python3
"""
Module to calculate Neural Style Transfer layer style cost
"""
import numpy as np
...
class NST:
    ...
    def layer_style_cost(self, style_output, gram_target):
        ...
        Parameters:
            style_output: tf.Tensor/tf.Variable of shape (1, h, w, c)
                          layer style output of the generated image
            gram_target: tf.Tensor/tf.Variable of shape (1, c, c)
                         gram matrix of target style output for that layer

        Returns:
            cost: tf.Tensor containing the layer's style cost
        """
        # style_output tensor növü və rank 4 yoxlanışı
        if not isinstance(style_output, (tf.Tensor, tf.Variable)) or \
           len(style_output.shape) != 4:
            raise TypeError("style_output must be a tensor of rank 4")

        # Kanal sayını (c) dinamik olaraq götürürük
        c = style_output.shape[-1]

        # gram_target tensor növü və düzgün forma (1, c, c) yoxlanışı
        ...
        # Generasiya edilən şəklin qat çıxışı üçün Gram matrisini tapırıq
        gram_style = self.gram_matrix(style_output)

        # İki matris arasındakı kvadratik fərqlərin ortalamasını (MSE) tapırıq
