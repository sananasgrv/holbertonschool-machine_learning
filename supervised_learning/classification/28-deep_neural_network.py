#!/usr/bin/env python3
"""Deep Neural Network for multiclass classification with activation option"""
import numpy as np
import pickle
import os
...
class DeepNeuralNetwork:
    ...
    def __init__(self, nx, layers, activation='sig'):
        """class constructor with activation function"""
    ...
    def gradient_descent(self, Y, cache, alpha=0.05):
        """performs gradient descent with activation"""
        m = Y.shape[1]
        ...
        for i in range(self.__L, 0, -1):
            ...
            if i > 1:
                ...
                if self.__activation == 'sig':
                    dz = np.matmul(W.T, dz) * (A_prev_activation * (1 - A_prev_activation))
                else:  # tanh
                    dz = np.matmul(W.T, dz) * (1 - A_prev_activation ** 2)
